...
// SQLite DB — ACID compliant, no JSON corruption risk
let sqldb = null;
const getOpenDB = () => {
    if (sqldb && sqldb.open) { // Check if db is open and valid
        return sqldb;
    }
    console.log("Re-initializing SQLite DB connection...");
...
function initDB() {
    // This initDB function is mostly for initial setup, subsequent calls should rely on getOpenDB()
    // It's called once at the top level
    getOpenDB(); // Ensure the database is initialized on startup
}
initDB();
...
const getDB = () => {
    return new Promise((resolve) => {
        getOpenDB().all("SELECT * FROM prophecies ORDER BY id ASC", [], (err, rows) => {
...
const saveDB = (data) => {
    return new Promise((resolve) => {
        const db = getOpenDB(); // Get the open DB instance once
        db.serialize(() => {
            db.run("BEGIN TRANSACTION");
            db.run("DELETE FROM prophecies", [], (err) => {
                if (err) {
                    db.run("ROLLBACK"); // Use db
                    console.error('DB clear error:', err.message);
                    resolve(); return;
                }
                const stmt = db.prepare(`INSERT INTO prophecies // Use db
...
function buildShareUrl(prediction, deadlineHuman, roast) {
    const cleanPred = prediction
        ...
        .replace(/[^\\w\\s.$]/g, '')
        .trim();
    const intro = `predicting this on @MonadMysticBot 🔮\\n\\n\"${cleanPred}\"\\nby ${deadlineHuman}\\n\\nthe AI's response: \"`;
...
async function runVerificationCycle(currentChatId, silent = false) {
    ...
    for (const p of db) {
        ...
        console.log(`⚡ Verifying #${p.id}: ${p.prediction}`);
        try {
            ...
            const finalResult = consensusCorrect ? result1 : { isCorrect: false, explanation: "Disputed verification — manual review required. *hic*" };
            ...
            if (finalResult.isCorrect && p.userWallet && !p.payoutSent) {
                ...
                console.log(`💰 Payout for #${p.id} (${p.payoutMethod}): ${payoutResult.success ? 'SUCCESS' : 'FAILED'}`);
            }
            ...
            if (targetChat) {
                bot.telegram.sendMessage(targetChat,
                    `${isWin ? '🎉' : '😂'} *PROPHECY #${p.id} ${isWin ? 'FULFILLED' : 'FAILED'}!*\\n\\n` +
                    `👤 *Prophet:* ${escapeMD(name)}\\n` +
                    `📜 *Prediction:* _\"${escapeMD(p.prediction)}\"_\\n` +
                    `📅 *Deadline was:* ${escapeMD(p.deadlineHuman)}\\n\\n` +
                    `${isWin ? '✅ THE ORACLE BOWS TO YOUR WISDOM!' : '❌ WRONG! The Oracle cackles into the void!'}\\n` +
...
async function checkProphecyById(id, ctx) {
    ...
    if (p.verified) {
        ...
        return bot.telegram.sendMessage(chatId,
            ...
            `🤖 Verdict: _\"${escapeMD(p.verificationResult?.explanation || 'No explanation')}\"_`,
            { parse_mode: 'Markdown' }
        );
    }
    ...
    bot.telegram.sendMessage(chatId,
        `🔍 *Prophecy #${id} has expired! Checking the timeline...*`,
        { parse_mode: 'Markdown' }
    );

    if (processingProphecies.has(id) || p.isProcessing) {
        return bot.telegram.sendMessage(chatId,
            `⏳ Prophecy #${id} is already being verified by the Oracle. Check back in a moment!`
        );
    }
    ...
    try {
        ...
        if (finalResult.isCorrect && p.userWallet && !p.payoutSent) {
            ...
            paidMsg = payoutResult.success ? `\\n\\n💸 *0.04 MON sent!*` : `\\n\\n⚠️ Payout failed`;
            if (!payoutResult.success) p.payoutFailed = true;
            console.log(`💰 Manual check payout for #${id} (${p.payoutMethod}): ${payoutResult.success ? 'SUCCESS' : 'FAILED'}`);
        }
...
bot.command('predict', (ctx) => {
    ...
    if (lastUsed && Date.now() - lastUsed < PREDICT_COOLDOWN_MS) {
        ...
        return ctx.reply(`⏳ The Oracle is still recovering from your last prophecy. Try again in ${secsLeft}s. *hic*`);
    }
    userCooldowns.set(ctx.from.id, Date.now());
    userStates.set(ctx.from.id, { step: 'PREDICTING' });
    ctx.reply(
        "📜 State your prophecy (e.g., 'Monad to $0.05 by Feb 15')...\\n\\n" +
...
bot.command('leaderboard', (ctx) => {
    ...
    if (board.length === 0) {
        return ctx.reply("📭 No prophecies yet. Be the first with /predict!");
    }
    ...
    board.forEach((p, i) => {
        ...
        msg += `   ✅ ${p.correct}/${p.total} — ${accuracy}% accuracy${pendingStr}\\n\\n`;
    });
...
// ADMIN: Manual payout override — protected + publicly announced for transparency
bot.command('adminpayout', async (ctx) => {
    if (!ADMIN_TELEGRAM_ID || ctx.from.id !== ADMIN_TELEGRAM_ID) {
        return ctx.reply("⛔ Only the Oracle's chosen admin can command this. *hic*");
    }
    const args = ctx.message.text.split(' ');
    ...
    if (targetChat) {
        bot.telegram.sendMessage(targetChat,
            `🛠️ *ADMIN OVERRIDE*\\nProphecy #${id} manually marked ${isCorrect ? 'CORRECT ✅' : 'INCORRECT ❌'} and processed. *hic*`,
            { parse_mode: 'Markdown' }
        );
    }
...
bot.on('text', async (ctx) => {
    ...
    if (state.step === 'PREDICTING') {
        ...
        return ctx.replyWithMarkdown(
            ...
            `⚠️ _Transaction must be made within the last 30 minutes_`
        );
    }
    ...
    if (state.step === 'PAYING') {
        ...
        if (!/^0x[a-fA-F0-9]{64}$/.test(txHash)) {
            userStates.delete(userId);
            return ctx.reply(
                "❌ *REJECTED!* That doesn't look like a valid TX hash. Must start with `0x` followed by 64 hex characters.",
                { parse_mode: 'Markdown' }
            );
        }
        ...
        if (!payResult.valid) {
            userStates.delete(userId);
            const reasons = {
                'ALREADY_USED': "❌ *REJECTED!* This TX hash was already used. Nice try, mortal. *hic*",
                'TOO_OLD': "❌ *REJECTED!* That transaction is too old. Must be within the last 30 minutes!",
                'WRONG_DESTINATION': "❌ *REJECTED!* That TX wasn't sent to the vault address.",
                'WRONG_AMOUNT': "❌ *REJECTED!* Must send exactly 0.01 MON, not more, not less.",
                'NOT_FOUND': "❌ *REJECTED!* Transaction not found on Monad. Check the hash.",
                'UNCONFIRMED': "❌ *REJECTED!* TX not yet confirmed. Wait ~10 seconds and try again, mortal.",
                'INVALID_FORMAT': "❌ *REJECTED!* Invalid TX hash format.",
                'ERROR': "❌ *ERROR!* Could not verify transaction. Try again."
            };
            return ctx.reply(
                reasons[payResult.reason] || "❌ *SACRIFICE REJECTED!* Try again.",
                { parse_mode: 'Markdown' }
            );
        }
        ...
        try {
            ...
            if (onChainResult && onChainResult.onChainId) {
                ...
                console.log(`📦 Prophecy #${id} stored on-chain with ID ${onChainResult.onChainId}`);
            }
            await saveDB(db);
        } catch(e) {
        ...
        const caption =
            ...
            `<i>\"${safeText}\"</i>\\n\\n` +
            `🎯 <b>Prediction:</b> ${safePred}\\n` +
            `⏰ <b>Deadline:</b> ${safeDeadline}\\n\\n` +
...
const startupCleanup = async () => {
    ...
    for (const p of db) {
        if (p.isProcessing && (now - new Date(p.timestamp).getTime() > 10 * 60 * 1000)) {
            p.isProcessing = false;
            cleaned = true;
            console.log(`🧹 Cleaned stale processing flag for prophecy #${p.id}`);
        }
    }
...
setInterval(async () => {
    if (ANNOUNCEMENT_CHAT_ID) {
        console.log("⏰ 5-minute auto-check triggered");
        const db = await getDB();
        const now = new Date();
